...
<script type="text/javascript">
	$(function(){
		...
		//为搜索操作模态窗口中的搜索框绑定事件，执行搜索操作并展现市场活动列表的操作
				...
				$.ajax({
					...
					success:function (data){
						...
						$.each(data,function (i,n){
							html += '<tr>';
							html += '<td><input type="radio" name="choose" value="'+n.id+'"/></td>';
		...
		//为提交市场活动按钮绑定事件,填充市场活动源(name+id)
		$("#submitActivityBtn").click(function () {

			//取得选中市场活动的id
			var choosed = $("input[name=choose]:checked");
			var id = choosed.val();
		...
		$("#convertBtn").click(function () {
			...
			/*	如果需要创建交易,除了要为后台传递clueId之外，还得为后台传递交易表单中的信息，金额，预计成交日期，交易名称，阶段，市场活动源(id)
				...
				以上传参的方式很麻烦，而且一旦表单扩充，挂载的参数优肯超出浏览器地址栏的上限
				使用提交交易表单的形式来发出本次传统请求
				提交表单的参数不用我们手动去挂载(表单中写name属性),另外，提交表单能够提交POST请求
			*/

				$("#tranForm").submit();
			}else {

				//在不需要创建交易的时候,传clueId就可以了
